...
export async function onRequestPost(context: EventContext<Env, string, unknown>) {
  ...
  const rawDeltas = Array.isArray(body?.puzzleDeltas)
    ? (body!.puzzleDeltas as DeltaRow[])
    : [];
  ...
  for (const delta of rawDeltas) {
    ...
    const puzzleType = normalizePuzzleType(
      delta.puzzleType ?? delta.puzzle_type,
    );
    if (!PUZZLE_TYPES.has(puzzleType)) continue;
    ...
    await context.env.ASCENT_DB.prepare(
      `INSERT INTO puzzle_positions
         (game_id, puzzle_type, fen, difficulty, view_count, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(game_id, puzzle_type, fen) DO UPDATE SET
         difficulty = MAX(0, puzzle_positions.difficulty + ?),
         view_count = MAX(0, puzzle_positions.view_count + ?),
         updated_at = ?`,
    )
      .bind(
        gameId,
        puzzleType,
        fen,
        Math.max(0, DEFAULT_PUZZLE_DIFFICULTY + dDelta),
        Math.max(0, vDelta),
        now,
        dDelta,
        vDelta,
